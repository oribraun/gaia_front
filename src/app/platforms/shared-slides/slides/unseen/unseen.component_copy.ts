...
export class UnseenComponent extends BaseSlideComponent implements OnInit {
    ...
    initMultipleOptions(q: any, question_index: number) {
        if (this.unseenAnswers[q.question_id].answer_text) {
            this.multiple_choice_answers[question_index] = {}
            let options = q.question.answers
            for (let i in options) {
                let is_correct = options[i].is_correct
                let answer_text = options[i].answer
                this.multiple_choice_answers[question_index][answer_text] = is_correct
    ...
    setResponseAnswer(data: any) {
        ...
        if(current_question.question_type == "multiple_choice" && !this.isEmpty(current_question.question.answers)){
            this.unseenAnswers[current_question.question_id].answer_text = this.multiple_choice_answers[this.question_index];
    ...
    checkAnswer(){
        ...
        const data = {
            "source": 'check_answer',
            "answer": current_question.question_type == 'multiple_choice' ? this.multiple_choice_answers[this.question_index] : this.unseenAnswers[current_question.question_id].answer_text,
    ...
    onMultipleChoiceQuestionChange(answer:any){
        if(this.multiple_choice_answers.hasOwnProperty(this.question_index)){
            let ans = this.multiple_choice_answers[this.question_index]
            if(ans.hasOwnProperty(answer.answer)){
                delete ans[answer.answer]
                delete this.checked[answer.answer + String(this.question_index)]
                if(this.isEmpty(ans)){
                    delete this.multiple_choice_answers[this.question_index]
                }
            } else {
            ...
            this.multiple_choice_answers[this.question_index] = {}
            this.multiple_choice_answers[this.question_index][answer.answer] = answer.is_correct
            this.checked[answer.answer + String(this.question_index)] = true
        }
    }
